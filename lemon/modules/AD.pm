...
sub createUser {
    ...
    my $sn = $req->data->{registerInfo}->{lastname};
    my $gn = $req->data->{registerInfo}->{firstname};

    if ($self->conf->{registerTransformNames}) {
        $sn = uc $sn;
        $gn = ucfirst $gn;
    }
    ...
    my $mesg = $self->ldap->add(
        ...
        attrs => [
            objectClass    => $self->conf->{registerLdapObjectClasses},
